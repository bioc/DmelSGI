\section{Mutli-phenotype genetic interaction cube}
...
$\pi$-scores are normalized through division by the median deviation to make them comparable across phenotypes. In this way $\pi$-scores are comparable to $z$-scores. \warning{Do not run the next code chunk. This will take quit a while. A hard coded result can be found in the overnext code chunk.}
